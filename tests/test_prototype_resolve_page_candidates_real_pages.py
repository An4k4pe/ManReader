"""Tests for the standalone resolve_page_candidates real-page prototype."""
...
SCRIPT_PATH = PROJECT_ROOT / "scripts" / "prototype_resolve_page_candidates_real_pages.py"


class PrototypeResolvePageCandidatesRealPagesTests(unittest.TestCase):
    def _run_script(
        self,
        pdf_bytes: bytes,
        *,
    ...
    def _plain_pdf_bytes(self) -> bytes:
        document = fitz.open()
        self.addCleanup(document.close)
        document.new_page(width=400.0, height=600.0)
    ...
    def test_page_number_out_of_range_is_explicitly_rejected(self) -> None:
        completed = self._run_script(self._plain_pdf_bytes(), page_number=2)

        self.assertEqual(completed.returncode, 3)
        result = json.loads(completed.stdout)
        self.assertEqual(result["category"], "PRECONDITION_FAIL")
        self.assertIn("page_number out of range", result["message"])
        self.assertEqual(result["input"]["page_count"], 1)
