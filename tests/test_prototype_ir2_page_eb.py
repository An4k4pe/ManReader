...
`Criterio_ConfrontoEB_v3.md`. Le due regole si testano qui e non nel modulo
...
class EmittedContentTest(unittest.TestCase):
    ...
    def test_it_drops_the_heading_hashes(self) -> None:
        ...
        emitted, losses = _emitted_content(page, frozenset())
    ...
    def test_it_drops_the_emphasis_delimiters(self) -> None:
        ...
        emitted, _losses = _emitted_content(PageIR2(page_id=PAGE, nodes=(node,)), frozenset())
    ...
    def test_an_excluded_node_does_not_reach_the_comparison(self) -> None:
        page = PageIR2(
            page_id=PAGE,
            nodes=(
        ...
        emitted, _losses = _emitted_content(page, frozenset({f"{PAGE}:n0"}))
        self.assertEqual(emitted, "Il corpo")
