...
def induci_con(livello: float):
    ...
    for pi, pg in enumerate(PAGES):
        idx = [i for i, l in enumerate(pg)
               if max(k[1] for _, k in l["spans"]) == livello]
    ...
    tp = sum(1 for z in zones if len(set(EZ[z])) >= 3
             and any(ob <= set(EZ[z]) for ob in SC) and vera(z))
    return {"zone": len(zones), "schemi": len(SC), "spiegate": spiegate,
            "tp": tp, "fp": spiegate - tp, "obbl": [sorted(o) for o in SC]}

print(f"\n{'livello':>8} {'zone':>6} {'schemi':>7} {'SPIEGATE':>9} {'(vere)':>7} {'(non vere)':>11}")
print("-"*56)
best, ris = None, {}
for s in LIVELLI:
    r = induci_con(s)
    ...
    print(f"{s:>8} {r['zone']:>6} {r['schemi']:>7} {r['spiegate']:>9} {r['tp']:>7} {r['fp']:>11}")
