...
Verifica del cambio con un invariante nuovo e più forte di quello di Milestone 36. **I2 — nessun riordino**: il cambio tocca dove si va a capo, non l'ordine, quindi la **sequenza** dei caratteri non-spazio letta a nastro deve restare identica carattere per carattere. Copre il buco che la riga 76 segnala nell'invariante di conservazione, che è un multiset e quindi cieco all'ordine — **ma solo per verificare un cambio, non come criterio di uscita**. I2 confronta la stessa pagina prima e dopo una modifica; il terzo invariante che manca alla milestone confronta l'ordine emesso con un **riferimento umano** su pagine trascritte a mano, e continua a non esistere. Le due cose non si sostituiscono: distinzione aggiunta dopo un rilievo della revisione indipendente, che le trovava contraddittorie. **Superata su tutte le pagine provate, confrontando ciascuna variante con se stessa prima e dopo il cambio** — non fra varianti diverse, che hanno ordini legittimamente diversi: DB p.53 (2.695 caratteri), Dag p.164 (4.144), DrW p.97 (2.132), DB p.50 (2.996), DB p.99 (1.230). **P0 è stato cambiato di proposito e dichiarato**: `page.md` non è più byte-identico, perché usava la stessa regola difettosa e tenercela avrebbe voluto dire conservare un artefatto di Milestone 36 che sappiamo sbagliato; può cambiare **solo** nelle interruzioni di paragrafo, e I2 è la prova.
...
**L'estensione dovrebbe valere verso il basso, non verso l'alto.** Posizione dell'utente, messa agli atti e **non implementata**. La ragione è di ordine di lettura e non geometrica: estendere un corridoio verso l'alto lo riporta in una parte di documento già letta, e lì la separazione che si presume non ha più nessuno da separare — mentre verso il basso segue il verso in cui il testo viene consumato. Emersa guardando DB p.53, dove il gutter della tabella si estende da 160 fino a 120, cioè dentro la fascia del box che sta sopra. Non è la stessa cosa della correzione sulla subordinazione, che è già stata fatta: quella toglie allo span il potere di **dimostrare** una nidificazione, questa cambierebbe **come lo span viene calcolato**, e tocca quindi anche le bande di primo livello. Nessuna misura è stata fatta: non si sa quante bande cambierebbero né in che direzione, e la simmetria attuale non è mai stata messa alla prova. Da aprire con criterio pre-registrato come i quattro di questa sessione, non prima.
...
**I quattro verdetti, scritti perché mancavano.** La revisione indipendente del 16 agosto 2026 ha rilevato che il verbale riportava precondizioni e predizioni — che i criteri stessi dichiarano non concorrenti al pass/fail — e **non** le regole di accettazione. Erano dati a voce nella sessione o non erano stati dati affatto; qui stanno per iscritto, con l'onestà di dire quali mancano.

| criterio | regola | esito |
| --- | --- | --- |
...
| 2, interruzione | A1 / A2 | **A1 CADUTA per la metà `embedded_visual`**, tenuta per la metà filetti. Su DB p.50 quella metà porta le primitive in banda da 83 a **0**: tornano a `(y0, x0)` su una pagina che ha gutter, che è esattamente il falso negativo che A1 vieta. Una versione precedente di questa riga registrava «non adottata», cioè una **decisione** al posto di un **verdetto**, e nascondeva una caduta. Rilievo della revisione architetturale. La decisione di non accendere il flag resta, ma è la conseguenza della caduta, non il suo sostituto. |
...
**I quattro verdetti sono ora dati, e uno è una caduta.** Giudizio dell'utente sul set di verifica a vista del 16 agosto 2026, nella sua formulazione: *«il testo è corretto come incolonnamento»*. Restano due difetti, entrambi **deferiti fuori da `column_band` perché non sono suoi**:
...
**Il limite di G1, dichiarato.** La conservazione promette che ogni gutter accettato compaia *esattamente una volta*: sparire e comparire due volte sono due direzioni. La prima è misurata e vale 0. **La seconda non è misurabile dall'uscita attuale**: le righe dell'albero identificano un gutter solo per intervallo x, e gutter distinti possono condividerlo — su Dag p.127 `399.0-408.0` compare sotto tre genitori nelle fasce y 82-270, 280-456 e 488-762, che sono tre gutter diversi emessi una volta ciascuno. Il tentativo di misurarla è stato il **quarto** errore di misura consecutivo di Chat A su questo invariante, nella stessa classe del primo. Per misurarla servirebbe che `_segment_tree` emettesse un'identità stabile per gutter. Rilievo della revisione indipendente, e la risposta è mezzo sì: la direzione mancava davvero, e non si può coprire senza toccare il producer.

**Difetto noto e NON corretto: lo span decide ancora dove va il subordinato.** `_is_subordinate` è passata agli estremi probatori (riga 799), ma la **collocazione** del subordinato dentro una colonna del padre no — `prototype_derived_column_bands.py:918-919` filtra ancora con `span_y0`/`span_y1`. Sono le due metà della stessa decisione prese con due grandezze diverse, ed è il difetto di DB p.53 un livello più in basso: una figlia entra in una banda che il suo span tocca e il suo estremo probatorio **non tocca affatto**. `Criterio_SubordinazioneProbatoria_v1.md` §1 dichiarava «nient'altro cambia» e non lo nominava.

Rilievo della revisione architetturale, che ne contava 23 pagine su 794 (14 manuali, apertura e corpo). **Riprodotto in indipendenza** con `scripts/find_span_placed_subordinates.py`: 10 casi su 7 pagine sui soli 5 manuali per 60 pagine dell'apertura — perimetro diverso, quindi i due numeri non si confrontano, ma il caso che la revisione cita per esteso esce identico cifra per cifra da entrambi i percorsi (DB p.31, banda 278-750, subordinato probatorio 86-204, span 80-282). Pagine: DB 9, DB 27, DB 31, DB 44, Dag 25, Dag 36, Fab 2.

**Guardato, ed è la quinta volta in questa sessione che un conteggio cade davanti a uno sguardo.** Giudizio dell'utente sulle sette pagine, con overlay: **sei si leggono correttamente**. L'unica in cui il difetto si manifesta è Fab p.2, e la causa non è la subordinazione — quella pagina ha **tre primitive con bbox fuori dal rettangolo pagina**, caratteri singoli con metriche rotte: `'z'` a bbox −43,7/−110,3 → 424,2/352,3, cioè 468×463pt per una lettera, `'O'` 269×273pt, `'y'` che sfora i 420×595 della pagina. Sono i «casi isolati con coordinate anomale su Fab.pdf p.2» già registrati più in alto in questa sezione, **dove però sono contati due e sono tre**. Il gap che parte in basso viene da un capolettera decorativo con metriche rotte, cioè da un difetto di cattura a monte.

Ne segue che **B4 è una incoerenza strutturale reale con severità pratica quasi nulla sul materiale osservato**: due metà della stessa decisione prese con due grandezze diverse, che su sei pagine su sette non producono alcun errore di lettura. Resta da chiudere perché è una incoerenza, non perché stia facendo danno.
...
**`column_band` è wired e il criterio regge sul percorso vero.** Chiuso il 17 agosto 2026 dopo una correzione: per qualche ora il verdetto è stato scritto più forte di come era stato ottenuto, perché il `page_bands.md` giudicato dall'utente **non era prodotto dal percorso wired** — rilievo della revisione architetturale, verificato. `scripts/prototype_vertical_slice_page.py` chiamava `_process_page` **dello script** diagnostico; `build_column_band_page_analysis` aveva tre chiamanti e nessuno produceva markdown. Due rami che non si toccavano, divergenti su 9 pagine su 20.

**Corretto con due modifiche, e la seconda ritira una mia prudenza infondata.** La fetta verticale chiama ora `column_band_tree` del **modulo di produzione**: una implementazione sola, ed è quella montata nel job. E il wiring emette **tutte** le bande, annidate comprese: la restrizione al primo livello era stata adottata contro il rischio che un consumatore ignaro dell'annidamento duplicasse contenuto o ne perdesse un livello, e quel rischio **non esiste nel consumatore che c'è** — l'assegnazione va alla banda più profonda, e su DB p.76, DB p.83, DB p.18 e DIE p.248 le primitive emesse due volte sono **zero**. Toglierla restituisce i gutter annidati che questa sezione assegna al consumer di tabelle («la descrizione corretta di una tabella a nove colonne»: DB p.76 torna a 7 gutter invece di 1).

**Rigenerato il campione cieco sul percorso corretto: sette pagine su sette identiche** a quelle già giudicate, byte per byte — DIE p.248, SV p.74, Dag p.141, Kul p.124, Dag p.197, Wil p.316, DrW p.50. Le altre tre non erano state generate prima e non producono bande. Il verdetto dell'utente vale quindi ora sul percorso che il job monta, che è ciò che mancava.

**La misura satellite esiste, e il ponte è caduto.** `page_analysis_column_band_measurements.py` (17 agosto 2026) implementa `ColumnBandMeasurements`, che Milestone 33 prevedeva e che non era mai stata scritta: porta `gutter_x_intervals`, `column_count`, `depth` e `parent_candidate_id`, cioè esattamente ciò che un `RegionCandidate` minimale non può portare. Stesso pattern di Milestone 7 — osserva, non classifica, non decide: la gerarchia è riportata come **fatto**, non come istruzione, e metterla in relazione resta di Resolution o del consumer.

**E il consumer ora legge il contratto, non le strutture interne del producer.** `scripts/prototype_vertical_slice_page.py` ricostruisce ciò che serve all'ordinamento da **candidati e misure** (`_tree_rows_from_contract`), non più dall'albero: dal candidato bbox e primitive, dalla misura i gutter e il livello. È la prova, rieseguita a ogni run, che il contratto di Milestone 33 **basta** a consumare — la domanda che quella milestone aveva lasciato aperta. Se un giorno quella funzione avesse bisogno di qualcosa che né candidato né misura portano, sarebbe la dimostrazione del contrario.

**Verifica**: le sette pagine del campione cieco già giudicate restano **identiche** dopo il passaggio, quindi il giudizio dell'utente vale ora su un percorso che non tocca l'albero interno. Sette test nuovi sulla misura, di cui quattro su stati impossibili che deve rifiutare. Suite 1263.

**Il debito precedente, superato.** `column_band_tree` è pubblica perché un `RegionCandidate` minimale porta bbox e primitive ma **non** i gutter né il livello: quelle grandezze Milestone 33 le assegnava a una misura satellite (`ColumnBandMeasurements`) che non esiste. Finché non esiste, un consumer che ordini per colonne non ha da dove prenderle, e l'alternativa sarebbe una seconda implementazione del meccanismo. `AGENTS.MD` §Milestone 33 registra come **scartata** l'opzione «più `RegionCandidate` senza misura satellite»: il wiring implementa quella scartata, con il ponte al posto della misura. Rilievo della revisione architetturale, accettato. `scripts/prototype_vertical_slice_page.py:849` chiama `_process_page` **dello script** e ordina sull'**albero pieno**; `build_column_band_page_analysis` ha tre chiamanti — il runner del job, se stesso, i propri test — e nessuno di essi produce markdown. Il wiring emette invece il solo primo livello. Divergenza misurata: **9 pagine su 20**, il 23,7% delle primitive in banda, e **una delle tre pagine del campione cieco** (DIE p.248, da 4 candidati a 3). W1/W2/W3 restano quindi **non applicati al wiring**: o si aggancia il consumer al producer e si rigiudica, o questa riga resta come sta.
...
### Chiusura definitiva, dopo lo scarico del §5

**`Criterio_ParagrafoDaRiga_v1.md` §5 è CADUTO** (`Esito_ParagrafoDaRiga_Par5_v1.md`,
commit `3e1a2f0`). È il primo criterio pre-registrato di questa sessione che
fallisce, ed è caduto facendo il suo mestiere: la clausola era rimasta non
scaricata alla chiusura, e scaricarla ha prodotto un verdetto negativo più un
difetto di contratto.

**Il verdetto**: su DB p.99 le righe degli stat block si spezzano **per campo** —
`Movimento: 8  Danno Bonus: —  PF: 8` è una riga sola sulla pagina e diventa tre
paragrafi. Giudizio dell'utente: più complesso da leggere. La segmentazione
peggiora, quindi il criterio non regge. **Sulla prosa invece migliora**, e il
verdetto non lo cancella: il box a rientro sospeso passa da quattro frammenti con
le frasi tagliate a tre voci corrette, e le parole spezzate si ricompongono.

**La regola non è sbagliata: è sbagliata dove il contenuto non è prosa.**

**Due difetti su DrW p.97 hanno una causa sola**, e nessuna era ovvia: i glifi dei
badge sono caratteri **minuscoli per la codifica** — `á` è U+00E1, il simbolo del
bersaglio è la lettera `o` — quindi la regola «si va a capo se la successiva non
comincia in minuscola» li legge come prosa che continua. La guardia elenchi copre
trattino e cifra, non i glifi di un font simbolico. La riserva del §6 del criterio
era ben posta: esercitata, non copre il caso reale.

**Difetto di contratto chiuso** (`f1878cc`): su DB p.53 l'ordinamento a bande
separa il glifo di elenco dal suo testo, quindi una riga di sorgente compare due
volte nell'ordine di lettura e due nodi collidevano sul `node_id`. Ora l'identità
viene dal primo primitivo. La base fa lo stesso split: il contratto l'ha soltanto
reso visibile invece di emettere una pagina malformata.

**Decisione presa** (`5bbb5f5`): gli asset il cui candidato Resolution non ha
accettato **non entrano nel corpo**. Il nodo continua a esistere — copertura per
costruzione — e a filtrare è la resa. Misurato: le note nel corpo passano da 75 a
10 sul campione cieco, cioè esattamente il conteggio della base. `review_ir2.md`
elenca ciò che resta fuori, raggruppato per genere: 55 `embedded_visual`, 6
`page_covering_visual`, 4 `page_edge_visual`. È il controllo da cui si guarderà
quando l'arredo verrà affrontato, o quando si verificherà se altri producer lo
risolvono già.

**Dag p.164 non è una regressione di IR 2**: la correzione esiste dietro
`--interrupt-corridor drawings`, spento per decisione di Milestone 37. Misurata
ora sulla metà filetti (`957ed1f`, flag diagnostico spento di default): su cinque
pagine non rompe nulla e ne aggiusta due — Dag p.164 sposta un nodo solo, DB p.53
fa leggere la tabella per righe invece che per colonne. Contraddice il «risolve un
caso solo» che l'aveva lasciato spento. **Non basta ad accenderlo**: sono pagine di
sviluppo, sul campione cieco l'unica che cambia è `Fab 127` con 42 nodi spostati e
non è stata guardata, e accendere il flag cambia l'ordine, quindi la base di E-B
andrebbe rigenerata **e rigiudicata**.
