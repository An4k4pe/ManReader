...
Decisioni aperte dopo la sessione del 15-16 agosto 2026, in aggiunta a quelle sotto: se implementare l'interruzione del corridoio nella forma che il discriminante suggerisce (un'immagine interrompe solo dove non vive testo), con criterio pre-registrato come i due di questa sessione; se far portare ai cluster `tiny` di `page_analysis_drawing_cluster_diagnostics.py` il proprio bbox, così che il consumer legga una classificazione invece di ridedurla; e se il reading order fra riquadri debba venire da `interior_visual_frame`, che li emette già.
