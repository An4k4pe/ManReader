...
Diagnostica pre-milestone per column_band, non ancora una milestone aperta. Storia in quattro fasi: la prima e la seconda hanno prodotto misure, la terza è stata fermata senza produrne, la quarta ha prodotto il meccanismo attuale.
...
Decisione bloccante lasciata aperta, riformulata in sessione dopo la revisione: **dove va la decisione "tabella ≠ colonna"**. Il fatto architetturale è verificato: `prototype_vertical_slice_page.py:84` importa `resolve_page_candidates`, quindi Resolution sta nella catena fra `column_band` e il reading order, e `State.md` chiama `column_band` un producer con contratto deciso in Milestone 33 — non siamo oltre i candidate, ci siamo dentro. `AGENTS.MD:154-155` stabilisce che un `RegionCandidate` è una proposta non approvata che non contiene semantica né decisione. Ne segue che i criteri di Fase 4 non sono tutti della stessa specie: `too_few_lines`, `too_short` e l'esclusione del testo ruotato rispondono a "esiste qui una separazione verticale fiancheggiata da testo", cioè **rilevazione**, e appartengono al producer; `too_few_chars` usato per scartare una tabella risponde a "questa è una tabella o una colonna", cioè **classificazione**, e appartiene a Resolution. La forma proposta: il producer emette il candidate con la firma già calcolata (caratteri per lato, conteggi di ruotati) e Resolution decide. Questo chiude il punto bloccante 2 di Milestone 33 invece di renderlo invisibile — rilievo di Chat B, che nota anche due precedenti ratificati in direzione opposta al filtro nel producer (Milestone 24 e Milestone 30), **non verificati da Chat A**.
...
**Principio che il repo pratica da tre milestone senza averlo mai enunciato, formulato dall'utente in questa sessione: fino ai candidati ragionare a pezzi isolati è corretto, dal consumer in poi mischiare gli approcci è inevitabile, perché una cosa influisce sull'altra.** Milestone 24 (relazione `page_edge_visual`/`side_band`), Milestone 30 (overlap `interior_visual_frame`/`table_candidate`) e Milestone 33 (overlap banda/`table_candidate`) hanno tutte e tre rinviato a Resolution/consumer, e `AGENTS.MD:440` chiama quel rinvio "lavoro futuro". Letti separatamente sembrano tre decisioni di comodo; letti insieme sono la stessa cosa strutturale — l'isolamento dei producer non è un limite da superare ma la precondizione perché la combinazione avvenga in un solo posto. Conseguenza operativa immediata: il confronto di reading order fatto qui è **incompleto**, perché collega `column_band` all'ordinamento ignorando gli altri quattro producer già disponibili. Il test corretto li usa tutti.
...
Diagnostica non wired, nessuna modifica a producer, contratti o wiring in nessuna delle quattro fasi — stesso standard delle altre milestone esplorative. Script di Fase 1 e Fase 2 committati in 786b547 (dieci script; nessuno per Fase 3, che non ne ha prodotti). Script di Fase 4 e del confronto fra i due meccanismi committati sul branch diagnostica-column-band-verifica, non ancora su main.
